            ...
            #   kind = 'lex-error'
                ...
                for i in range(len(value)):
                    if value[i] in reserve_symbol:
                        ...
                        error = (
                            f'Lexical Error on Ln {line_num}, Col {column}: Unexpected Illegal Character {value!r}')
                        kind = 'lex-error'
                continue
# ----------------------------------------------------------------------------------------------------------
            ...
            if len(value) > 15:
                error = (
                    f'Lexical Error on Ln {line_num}, Col {column}: Identifier exceeded a max length of 15 \ncharacters, you inputted {len(value)} characters')
                kind = 'lex-error'
            else:
...
# ----------------------------------------------------------------------------------------------------------
            ...
            if re.search(r'(\'\\\'\')|(\'[ -&\(-\[\]-~]\'$)', value):
                ...
                kind = 'lex-error'
                error = (
                    f'Lexical Error Ln {line_num}, Col {column}): Char literal is unterminated')
            elif re.search(r'\'$', value):
                kind = 'lex-error'
                error = (
                    f'Lexical Error Ln {line_num}, Col {column}: Char literal exceeded a max length of 1, you inputted {len(value)-2} character/s')
# ----------------------------------------------------------------------------------------------------------
            ...
            if (len(value)-2 > 1 if re.search(r'\"$', value) else len(value)-1 > 1):
                ...
                if re.search(r'(\"[ -!#-\[\]-~]*?(\\\")+?[ -!#-\[\]-~]*?\"$)|(\"[ -!#-\[\]-~][ -!#-\[\]-~]+?\"$)', value):
                    ...
                    kind = 'lex-error'
                    error = (
                        f'Lexical Error Ln {line_num}, Col {column}): String literal is unterminated')
            else:
                kind = 'lex-error'
                error = (
                    f'Lexical Error Ln {line_num}, Col {column}: Str literal minimum character length is \n2 characters, you inputted {len(value)-2} character')
# ----------------------------------------------------------------------------------------------------------
            ...
            if '!' in value and len(value) <= 10:
                ...
                if re.search(r'^(!0+)$', value):
                    error = (
                        f'Lexical Error on Ln {line_num}, Col {column}: Negative Zero Error')
                    kind = 'lex-error'
                else:
                ...
                error = (
                    f'Lexical Error on Ln {line_num}, Col {column}: Int literals exceeded a max length of 9, \nyou inputted {len(value)-1 if "!" in value else len(value)} digits')
                kind = 'lex-error'
# ----------------------------------------------------------------------------------------------------------
        elif '.' in value:
            # Deci
            if kind == 'number' and re.search(r'\.[0-9]+$', value):
                ...
                if length <= 10 and '!' in value and len(value) - length <= 10:
                    ...
                    error = (
                        f'Lexical Error on Ln {line_num}, Col {column}: Deci literals exceeded in left handside \na max length of 9, you inputted {length-1 if "!" in value else length} digits')
                    # Lex Error - Deci_literal - Left handside
                    kind = 'lex-error'
                elif (length-1 if '!' in value else length) <= 9 and len(value) - length > 10:
                    error = (
                        f'Lexical Error on Ln {line_num}, Col {column}: Deci literals exceeded in right handside \na max length of 9, you inputted {len(value) - length - 1} digits')
                    # Lex Error - Deci_literal - Right handside
                    kind = 'lex-error'
                else:
                    error = (
                        f'Lexical Error on Ln {line_num}, Col {column}: Deci literals exceeded a max length of 18,\nyou inputted {len(value)-2 if "1" in value else len(value)-1} digits')
                    # Lex Error - Deci_literal
                    kind = 'lex-error'
            elif kind == 'number':
                error = (
                    f'Lexical Error on Ln {line_num}, Col {column}: Incomplete deci literal')
                kind = 'lex-error'
                pass
            else:
                error = (
                    f'Lexical Error on Ln {line_num}, Col {column}: Invalid deci literal')
                kind = 'lex-error'
